...
export const deleteClient = [
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const cachedClients = await redisClient.get(cacheKeyList);
      if (cachedClients) {
        const clients = JSON.parse(cachedClients).filter(
          (c: any) => c._id !== clientId
        );
        await redisClient.setEx(
          cacheKeyList,
          CACHE_TTL,
      ...
      logError('Error deleting client', {
        error,
        agencyId: req.agencyId,
...
export const getAllClients = [
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const cacheKey = `clients:${req.params.agencyId}:${pageNum}:${limitNum}`;
      const { data: clients, cached } = await cacheOrQuery(
        cacheKey,
        () =>
          Client.find({ agencyId: req.params.agencyId })
...
export const getClientById = [
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const cacheKey = `client:${req.params.agencyId}:prefId:${preferencesId}`;
